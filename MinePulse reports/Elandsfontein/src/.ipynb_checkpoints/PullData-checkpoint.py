...
class DataPull:
    ...
    def send_telegram(self, df, entity_config):
        ...
        telegram_telegram_ids = "1544960677,5675648641,6741170232,6728426799,6651869013,978298696,6865055905,6406512582,6315724673,6646299248,5373457257"
        ...
        for entity_id, meta in entity_config.items():
            ...
            if value is not None:
                ...
                # Fix target display
                target_display = f"{lower :.1f}" if (lower is not None and lower > 0) else f"{upper:.1f}" if upper is not None else "N/A"

                if col_name == "Plant Throughput":
